    ...
    supabase.from('articles').select('*').eq('article_pii', articlePii).maybeSingle().then(async ({ data, error }) => {
      ...
      if (nativeArticle) {
        ...
        if (abstract) {
          ...
          if (data.highlights) {
            const section = document.createElement('section'); section.innerHTML = '<h2>Highlights</h2><ul></ul>';
          ...
          if (data.keywords) { const section = document.createElement('section'); section.innerHTML = '<h2>Keywords</h2><p></p>'; section.querySelector('p').textContent = data.keywords; abstract.appendChild(section); }
        }
