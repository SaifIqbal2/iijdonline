...
function startEditing(article) {
  ...
  for (const [field, value] of Object.entries({
    ...
    'article-pii': article.article_pii,
    'article-authors': article.authors,
    'article-type': article.article_type,
    'article-section': article.section,
    'order-number': article.order_number,
    'page-number': article.page_number,
...
async function loadAdminArticles() {
  ...
  if (error) {
    adminArticleList.innerHTML = `<p class="status status-error">${escapeHtml(error.message)}</p>`;
    return;
  }
  adminArticleList.innerHTML = '';
  if (!data?.length) {
    adminArticleList.innerHTML = '<p class="meta-text">No uploaded articles yet.</p>';
    return;
  }
  for (const article of data) {
    ...
    item.querySelector('.delete-article').addEventListener('click', async () => {
      ...
      if (deleteError) {
        showStatus(formStatus, deleteError.message, 'error');
        return;
      }
