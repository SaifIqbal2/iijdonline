(() => {
  ...
  root.style.cssText = 'display:block!important;width:100%!important;max-width:none!important;float:none!important;column-count:1!important;';
  ...
  supabase.from('articles').select('*').eq('article_pii', articlePii).maybeSingle().then(async ({ data, error }) => {
    ...
    root.innerHTML = `<style>.supabase-live-preview{display:block!important;width:100%!important;max-width:1100px!important;margin:0 auto!important;padding:28px 36px 60px!important;box-sizing:border-box!important;color:#1d2a39;font-family:Arial,sans-serif}.supabase-live-preview h1{color:#164f73;font-size:2rem;line-height:1.2}.supabase-live-preview h2{color:#164f73;margin-top:28px}.supabase-live-preview .preview-meta{color:#526477;margin:10px 0}.supabase-live-preview .preview-authors{font-weight:600}.supabase-live-preview .preview-abstract{line-height:1.7;white-space:pre-line}.supabase-live-preview .preview-pdf{display:inline-block;margin-top:22px;padding:11px 16px;background:#17618b;color:#fff;text-decoration:none;font-weight:700}</style><div class="supabase-live-preview"><div class="preview-meta">${data.article_type || 'Article'} | Volume ${data.volume || 'N/A'} | Issue ${data.issue || 'N/A'}</div><h1></h1><p class="preview-authors"></p><div class="preview-meta">Published online: ${data.published_at || 'Date not set'}</div><h2>Abstract</h2><div class="preview-abstract"></div>${highlights.length ? '<h2>Highlights</h2><ul class="preview-highlights"></ul>' : ''}${keywords.length ? '<h2>Keywords</h2><p class="preview-keywords"></p>' : ''}${pdfUrl ? `<a class="preview-pdf" href="${pdfUrl}" target="_blank" rel="noreferrer">Open PDF</a>` : ''}</div>`;
