    ...
    supabase
      ...
      .then(async ({ data, error }) => {
        ...
        article.innerHTML = `
          ...
          <div class="supabase-preview">
            <div class="preview-meta">${data.article_type || 'Article'} | Volume ${data.volume || 'N/A'} | Issue ${data.issue || 'N/A'}</div>
            <h1>${data.title}</h1>
            <p class="preview-authors">${data.authors || 'Authors not provided'}</p>
            <div class="preview-meta">Page ${data.page_number || 'N/A'} | Order ${data.order_number ?? 0} | Published ${data.published_at || 'Date not set'}</div>
            <h2>Abstract</h2>
            <div class="preview-abstract">${data.abstract || 'No abstract available.'}</div>
            ${highlights.length ? `<h2>Highlights</h2><ul>${highlights.map((item) => `<li>${item}</li>`).join('')}</ul>` : ''}
            ${keywords.length ? `<h2>Keywords</h2><p class="preview-keywords">${keywords.join(', ')}</p>` : ''}
            ${pdfUrl ? `<a class="preview-pdf" href="${pdfUrl}" target="_blank" rel="noreferrer">Open PDF</a>` : ''}
          </div>`;
      });
  }
